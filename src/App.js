  ...
  useEffect(() => {
    const fetchArticles = async () => {
      const res = await fetch(
        `https://api.nytimes.com/svc/search/v2/articlesearch.json?q=${theme}&api-key=OxcFkIOim3cHY1YY0SxygD1LullYrkm5`
      );
      ...
      <section className="grid grid-cols-1 gap-10 px-5 pt-10 pb-20">
        {articles.map((article) => {
          ...
          return (
            <article
              key={_id}
              className="bg-white py-10 px-5 lg:w-6/12 lg:mx-auto"
            >
